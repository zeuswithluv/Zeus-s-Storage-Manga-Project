...
1. **KHÔNG BAO GIỜ hardcode email cá nhân vào code**:
   ...
   - *Cách đúng:* Sử dụng Environment Variables (`import.meta.env.VITE_ADMIN_EMAIL`) hoặc kiểm tra qua Database (bảng `admins`).
...
## 5. 🛠️ Khắc phục các lỗi thường gặp

### Lỗi Đăng nhập (Auth Error)
Nếu bạn nhấn Đăng nhập mà không có gì xảy ra hoặc báo lỗi:
1.  **Authorized Domains**: Truy cập [Firebase Console](https://console.firebase.google.com/) -> Chọn dự án -> **Authentication** -> **Settings** -> **Authorized domains**. 
    - Nhấn "Add domain" và thêm tên miền Vercel của bạn (ví dụ: `my-manga-storage.vercel.app`).
2.  **Biến môi trường trên Vercel**: 
    - Vào Vercel Dashboard -> Settings -> Environment Variables.
    - Đảm bảo đã nhập đủ các biến từ file `.env` (VITE_FIREBASE_API_KEY, v.v...).
3.  **Google Cloud Console**: Nếu bạn đã bật "API Restrictions" cho API Key, hãy đảm bảo đã cho phép "Identity Toolkit API" và "Google People API".
